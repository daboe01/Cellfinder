...
// fixme: scale support
// draw dots in layer instead of making them "full blown" views to improve performance
// support lines
...
+(CPArray) getFrameForDataArray:(CPArray) theArr
{	return CPMakeRect(0,0,100,100);	// <!> fixme
}
...
{	var mouseLocation = [self convertPoint:[event locationInWindow] fromView:nil];
	...
	{	if([event modifierFlags])	// build the marqueeRect
		...
		{	[self addToModelPoint: mouseLocation];
...
{	var myArr=[self objectValue];
	...
	var myNewPoint=[[myArr entity] createObjectWithDictionary: myDict];
	[myArr addObject: myNewPoint];
}
